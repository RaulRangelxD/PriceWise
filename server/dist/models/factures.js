...
export const getAllFacturesByUserIdAndPaginationModel = async (user_id, limit, offset) => {
    const result = await db.execute({
        sql: `
      SELECT f.*, c.name AS company_name, 
      COALESCE(GROUP_CONCAT(p.name || ' ' || fp.quantity, ', '), '') AS products
...
export const getFacturesByDateRangeModel = async (user_id, startDate, endDate) => {
    const result = await db.execute({
        sql: `
      SELECT f.*, 
...
export const getFacturesByDateRangeAndCompanyIdModel = async (company_id, startDate, endDate) => {
    const result = await db.execute({
        sql: `
      SELECT f.*, 
      COALESCE(GROUP_CONCAT(p.name || ' (x' || fp.quantity || ')'), '') AS products

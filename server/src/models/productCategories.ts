...
export const deleteProductCategoryModel = async (id: string) => {
  const result = await db.execute({
...
export const deleteProductCategoryByProductIdAndCategoryIdModel = async (product_id: string, category_id: string) => {
  const result = await db.execute({

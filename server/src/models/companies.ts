...
export const getAllCompaniesByUserIdModel = async (user_id: string) => {
  const result = await db.execute({ sql: `SELECT * FROM companies WHERE user_id = :user_id`, args: { user_id } })
  return result
}

export const getAllCompaniesByUserIdAndPaginationModel = async (user_id: string, limit: number, offset: number) => {
...
export const getCompanyByIdModel = async (id: string) => {
  const result = await db.execute({ sql: `SELECT * FROM companies WHERE id = :id`, args: { id } })
  return result
}

export const postCompanyModel = async (user_id: string, name: string, rif: string, phone: string, address: string) => {
...
export const patchCompanyModel = async (name: string, rif: string, phone: string, address: string, id: string) => {
  const result = await db.execute({
...
export const deleteCompanyModel = async (id: string) => {
  const result = await db.execute({

...
export const FacturesTable = ({ companyIdInParams, rows, rowsDefault }: FacturesTableProps) => {
  const { userInContext } = useAuth()
  const router = useRouter()
  const [facturesData, setFacturesData] = useState<FactureData[] | null>(null)
  ...
  const getData = useCallback(async () => {
    try {
      ...
      if (companyIdInParams) {
        const facturesDataResult = await getAllFacturesByCompanyIdAndPagination(companyIdInParams, rowsPerPage, currentPage - 1)
  ...
  }, [userInContext, companyIdInParams, rowsPerPage, currentPage])
            ...
            <tbody>
              {facturesData?.map((facture, index) => (
                <tr
                  ...
                  } relative w-full hover:bg-opacity-75 dark:hover:bg-opacity-75 group ${index !== facturesData.length - 1 ? 'border-b border-default-light dark:border-default-dark' : ''}`}
                >
                  ...
                  <td className='px-2 py-2 text-nowrap max-w-20 md:max-w-36 lg:max-w-60'>
                    <p className='text-ellipsis overflow-hidden'>{facture.date}</p>
                  </td>
                </tr>
              ))}
            </tbody>

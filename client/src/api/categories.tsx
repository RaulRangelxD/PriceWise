...
export const getAllCategoriesByUserIdAndNotProductId = async (userId: string, productid: string): Promise<CategoryData[]> => {
  try {
    const result = await axios.get(`${BASE_URL}/userid/notproductid/${userId}`, { params: { productid }, headers: { 'Cache-Control': 'no-store' }, withCredentials: true })
    return result.data.data
  } catch (error) {
    console.error(error instanceof Error ? error.message : 'An unexpected error occurred')
    throw error
...
export const getAllCategoriesByProductId = async (productid: string): Promise<CategoryData[]> => {
  try {
    const result = await axios.get(`${BASE_URL}/userid/productid/${productid}`, { params: { productid }, headers: { 'Cache-Control': 'no-store' }, withCredentials: true })
    return result.data.data
  } catch (error) {
    console.error(error instanceof Error ? error.message : 'An unexpected error occurred')
    throw error
